...
缓存。安装器仅用内置 0.3.6 wheel 创建用户级隔离环境和受管理的 launcher，输出 JSON
...
事务性更新到 0.3.6。升级成功后，项目只保留 `.agents/skills/codex-team`，并移除旧的
`.agents/skills/global-scheduler`、`.agents/skills/codex-team-staff` 和
`.agents/skills/parlant-staff-shorthand`。裸 `codex-team`/`start` 会先自动执行这个更新，成功后才
