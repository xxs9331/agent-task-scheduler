...
def test_that_two_projects_isolate_lifecycle_and_observation_failure(
    ...
    alpha = _create_project(tmp_path / "alpha", project_id="alpha", events_path=".scheduler")
    ...
    alpha_publish = _run(
        capsys, ["--project-root", str(alpha), "publish", "--from-file", str(alpha_envelope)]
    )
    beta_publish = _run(
        capsys, ["--project-root", str(beta), "publish", "--from-file", str(beta_envelope)]
    )
    next_receipt = _run(capsys, ["--project-root", str(beta), "next", "--worker", "worker"])
    claim_receipt = _run(
        capsys,
        ["--project-root", str(beta), "claim", "--task", "beta-task", "--worker", "worker"],
    )
    ...
    assert len(alpha_state["publish_history"]) == len(beta_state["publish_history"]) == 1
