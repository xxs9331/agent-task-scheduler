...
def _candidate(cache: Path, version: str = "0.4.2") -> dict[str, object]:
    ...
    marker = {"managed_skill": "codex-team", "version": version, "files": {"SKILL.md": sha256(tracked.read_bytes()).hexdigest()}}
    (skill / "assets" / "managed-skill.json").write_text(json.dumps(marker), encoding="utf-8")
    with zipfile.ZipFile(wheel, "w") as archive:
        archive.writestr(f"agent_task_scheduler-{version}.dist-info/METADATA", f"Metadata-Version: 2.1\nVersion: {version}\n")
    return {"plugin": "global-scheduler", "version": version, "skill_root": str(skill), "wheel": str(wheel)}
    ...
    *,
    ...
    def run(command: object) -> subprocess.CompletedProcess[str]:
        ...
        if arguments[-2:] == ["--prefix", str(prefix)]:
            ...
            return subprocess.CompletedProcess(arguments, code, json.dumps({"ok": code == 0}), "")
...
def test_strict_version_rejects_same_downgrade_and_malformed() -> None:
    assert is_strictly_newer("0.4.2", CURRENT_VERSION)
...
def test_candidate_requires_identity_hashes_paths_and_one_matching_wheel(tmp_path: Path) -> None:
...
def test_off_avoids_runner_and_auto_requires_restart(tmp_path: Path) -> None:
    ...
    result = UpdatePreflight(tmp_path / "prefix", tmp_path, lambda _command: (_ for _ in ()).throw(AssertionError())).check()
    assert result["checked"] is False
    write_policy(tmp_path / "prefix", "auto")
    candidate = _candidate(tmp_path)
    prefix, project = tmp_path / "prefix", tmp_path / "project"
    result = UpdatePreflight(prefix, tmp_path, _transaction_runner(candidate, prefix=prefix, project=project), project).check()
    assert result["restart_required"] is True
    assert result["project_updated"] is True


def test_network_and_subprocess_failures_preserve_local_installation(tmp_path: Path) -> None:
    result = UpdatePreflight(tmp_path / "prefix", tmp_path, _runner({}, returncode=1)).check()
    assert result["ok"] is True
    assert result["warning"] == "UPDATE_CHECK_UNAVAILABLE"


def test_auto_updates_the_validated_launcher_and_current_project_only(tmp_path: Path) -> None:
    candidate = _candidate(tmp_path)
    prefix, project = tmp_path / "prefix", tmp_path / "project"
...
def test_launcher_failure_restores_the_last_validated_local_state(tmp_path: Path) -> None:
    candidate = _candidate(tmp_path)
    prefix, project = tmp_path / "prefix", tmp_path / "project"
    ...
    write_policy(prefix, "auto")
    result = UpdatePreflight(prefix, tmp_path, _transaction_runner(candidate, prefix=prefix, project=project, fail_stage="launcher"), project).check()

    assert result == {"ok": False, "operation": "update-preflight", "code": "UPDATE_TRANSACTION_FAILED", "stage": "launcher", "rolled_back": True, "project_updated": False}
...
def test_project_failure_restores_launcher_and_project_state(tmp_path: Path) -> None:
    ...
    write_policy(prefix, "auto")
    result = UpdatePreflight(prefix, tmp_path, _transaction_runner(candidate, prefix=prefix, project=project, fail_stage="project"), project).check()
